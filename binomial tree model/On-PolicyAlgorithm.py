...
class AgentQRDQN:
    ...
    def huber_lossfunction(self,target,tf_approx,action):
        ...
        huber_loss = self.huber_loss(tf.expand_dims(target, -1), tf.expand_dims(theta_tobeupdated, -1))
        error = tf.math.subtract(target, theta_tobeupdated)
        ...
        loss= tf.where(tf.less(error, 0.0),inv_tau*huber_loss,tau*huber_loss)
    ...
    def make_action(self,state,step,ifrandom): # the eponsion can be changed
        ...
        if t == self.MAX_t-1:
            ...
            if (np.random.rand() < eps) and ifrandom:
                ...
                state_boolean[0][self.MAX_t+state[1]] = 1
                state_boolean[0][self.MAX_t+self.MAX_S+state[2]] = 1
                quantile = self.q.predict([state_boolean])[0]
                return np.argmax(quantile[:,0])
        
def run_RL(env,agent,number_of_steps,start_state,OptionIniP):
    state = env.start_state
    trajectory = [[(0,0,0),0,0,(0,0,0)] for j in range(env.MAX_t)]
    loss_vec = np.array([]) 
    for i in range(number_of_steps):
        if i < 0.8*number_of_steps:
...
trained_model,loss_vec = run_RL(envGBM,QRDQN,number_of_steps,start_state,OptionIniP)
start_state_boolean = np.zeros((1,MAX_t+MAX_S+MAX_H))
start_state_boolean[0][start_state[0]] = 1
start_state_boolean[0][MAX_t+start_state[1]] = 1
start_state_boolean[0][MAX_t+MAX_S+start_state[2]] = 1
...
#using Monte Carlo method to evaluate the policy obtained by the RL Algorithm above
def optimal_action(state,model,MAX_t,MAX_S,MAX_H):
    t,S,H = state
    if t == MAX_t-1:
      action =  0  # in the last step, clean the portfolio
    else:
      state_boolean = np.zeros((1,MAX_t+MAX_S+MAX_H))
      state_boolean[0][state[0]] = 1
...
def make_action(state,MAX_t,SLowerBd):
    t,S,H = state
    if t == MAX_t-1:
        action = 0  # in the last step, clean the portfolio
    elif t == 0:
        ...
        if S+SLowerBd==53 or S+SLowerBd==51:
            action = 4
        elif S+SLowerBd==49 or S+SLowerBd==47:
            action = 0              
        
    return action    

OptionIniP = 3/4           
number_of_experiments = 10**3
state = start_state
trajectory = [[(0,0,0),0,0,(0,0,0)] for j in range(MAX_t)]
results = np.array([])
for i in range(number_of_experiments):
    action = optimal_action(state,QRDQN.q,MAX_t,MAX_S,MAX_H)
    #action = make_action(state,MAX_t,SLowerBd)
    reward,new_state = envGBM.step(action,OptionIniP)
    trajectory[state[0]][0] = state
    trajectory[state[0]][1] = action
    trajectory[state[0]][2] = reward
    trajectory[state[0]][3] = new_state
