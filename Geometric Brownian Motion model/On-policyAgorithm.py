...
class Env:
    ...
    def step(self,action,r,sigma,delta_t,OptionIniP):
        t,S,H = self.state
        t += 1
...
def run_RL(env,agent,number_of_steps,r,sigma,delta_t,start_state,OptionIniP):
    state = env.start_state
    trajectory = [[(0,0,0),0,0,(0,0,0)] for j in range(env.MAX_t)]
    loss_vec = np.array([])
 
    for i in range(number_of_steps):
        if i < 0.95*number_of_steps:
            action = agent.make_action(state,i,True)
        else:
            action = agent.make_action(state,i,False)
        reward,new_state = env.step(action,r,sigma,delta_t,OptionIniP)
...
dPlus = (np.log(S0/K)+(r+sigma*sigma/2)*TimeRemain)/(sigma*TimeRemain**(1/2))
...
plt.title("Training Loss of neural networks",fontsize=25)
