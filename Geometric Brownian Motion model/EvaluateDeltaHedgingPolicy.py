...
class AgentQRDQN:
    ...
    def make_action(self,state,K,r,sigma,delta_t):
        t,S,H = state
        if t == self.MAX_t-1:
...
def run_RL(env,agent,number_of_steps,r,sigma,delta_t,start_state):
    ...
    for i in range(number_of_steps):
        action = agent.make_action(state,K,r,sigma,delta_t)
...
plt.figure(1, figsize=(12,8))
plt.grid()
plt.hist(quantileVec[:,0])
plt.title("0.1 Quantile",fontsize=25)
plt.show()
plt.figure(1, figsize=(12,8))
plt.grid()
plt.hist(quantileVec[:,2])
